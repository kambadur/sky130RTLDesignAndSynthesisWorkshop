...
iVerilog in short to [Icarus Verilog](http://iverilog.icarus.com/) is an open source toolchain for simulation and synthesis. Although it is used only for simulation due to the potential advantages Yosys brings as a synthesis tool (*more details later*). iVerilog frameowrk requires the RTL desgin file and a test bench file for simulation.  
...
[GTKWave](http://gtkwave.sourceforge.net/) is an open source tool for visualizing the signal dumps in .vcd format.  
